...
MODLOADER_DIR = resource_loader.base_dir.joinpath( 'modloader' )
GTA_SA_FILE = resource_loader.base_dir.joinpath( 'gta_sa.exe' )
MODLOADER_FILE = MODLOADER_DIR.joinpath( 'modloader.ini' )
...
def get_modloader_text(mode_list=False):
    if MODLOADER_FILE.exists():
...
def get_folder_config_line_number():
    '''
    ...
    for line in init_text:
        if line.replace(' ', '').startswith('[Folder.Config]'):
            if folder_config_line_number == None:
                folder_config_line_number = line_number
        line_number += 1

    return folder_config_line
...
def get_current_profile():
    ...
    '''
    folder_config_line_number = get_folder_config_line_number()
    profile = None
    ...
    if not folder_config_line == None:
        ...
        while loop:
            line_number += 1
            current_line = folder_config_line_number + line_number
            if current_line < len(text_ini):
                text_line = ignore_comment( ini_text[current_line], ';' ).replace(' ', '')
...
def set_profile(profile=None, ini_text=None):
    '''
    Cambiar perfil
